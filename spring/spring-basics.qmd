...
**Spring Framework is a powerful lightweight application development framework** used for Enterprise Java (Java EE).
...
As we have seen, **IBM** set the foundations.` Java EE` and `EJB` were the first frameworks trying to solve the problems around building *enterprise Java applications*.

`J2EE`, `Java Enterprise Edition` or `EJB` that preceded `Spring` tried to do the same thing.

Then, the `EJB` and `J2EE` frameworks were to be surpassed by `Spring`.

The first try was written by **Rod Johnson**, who <mark>contributed to the understanding of `J2EE` development practices and with the publication of his book [Expert One-on-One J2EE Design and Development](/books/j2ee.qmd) in October 2002 mean to develop without `EJB`</mark>.
...
`Spring` provided a more lightweight and flexible alternative to the heavy, XML-configured, and often cumbersome `J2EE` frameworks of the time.
