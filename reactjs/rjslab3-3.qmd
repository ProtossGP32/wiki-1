...
![General schema){.external}](/images/reactjs/lab-re-3-14.png){}

It also provides the necessary functions:
...
to manipulate the ``Todo`` items through the ``ApiContext``, which is created using React's createContext hook.

The ``Todos`` component renders the ``CreateTodo`` component and the ``ReadTodos`` component within a container. 
...
| -- | -- | -- | -- |
| todoApp 0.3 | add ToDoGrid component to project: [todoApp 0.3](){.external} | [initial tree-folder](/images/reactjs/lab-re-3-5.png){width="60%"} | [render todoGrid](/images/reactjs/lab-re-3-4.png){width="80%"} |
