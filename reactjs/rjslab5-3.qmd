---
title: "Lab#RE05-3: chat & ws: front"
...
5. `reducer`: areducer function called conversationReducer that handles different actions:
   1. When the action type is "send", it creates a data object with information like action, chatId, userId, and text.
   2. If ready is true, it sends the data as a string. It then returns a new state array with a new message object appended.
   3. When the action type is "receive", it parses the message and creates a new message object with properties like id, time, chatId, userId, and text. It returns a new state array with the new message object appended.
   4. When the action type is "login", it parses the message and creates a new message object with properties like id, userId, time, and text. The text is constructed using different properties from the parsed message.
   5. It returns a new state array with the new message object appended.
   6. For any other action type, it simply returns the current state unchanged.
1. `useEffect`: it runs when the message variable changes.
...
2. Packing `props` to tree-component downstairs
3. Render
4.  `function ConversationsList`
