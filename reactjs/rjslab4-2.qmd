...
Operation | Path | Query | JSON | Description |
| -- | -- | -- | -- | -- |
| GET | `/todo` | `/` |  {`todo items`} | Retrieves a list of all todo items. |
| POST | `/todo` | `/createTodo` | {`todo item`} **body/raw/JSON** | Creates a new todo item. |
| DELETE | `/todo` | `/?id=4` | `id` as a **param** | Deletes all todo items. |
