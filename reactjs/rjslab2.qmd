...
> The combination of (1) the `useEffect` hook (which synchronizes execution with two different sources) and (2) the render cycle trigger-render-paint (due the change of state) **are the core business-logic of this code.**

The key-idea of the `useEffect` in this particular use (related to a timer problem) is **the synchronization of the execution with two different sources** and **when and who call the render-cycle**:
