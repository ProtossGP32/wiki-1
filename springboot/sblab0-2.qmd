...
subtitle: Spring Boot Library Management CRUD User (and Librarian)
author: albertprofe
date: "06/01/2021"
...
1. The client sends a ``request`` to server-controller by an ``endpoint`` to get the update-user form: [Update user template](){.external}

```html
<td>
     <a th:href = "@{packedForm(userIdFromView=${user.userId})}"
     >Update this book</a>
</td>
```
::: {.column-margin}
...
2. The server-controller method handles the ``request`` and sends a response with the update-user form: [UserController](){.external}

```java
@RequestMapping("/packedForm")
public String packedForm(@RequestParam("userIdFromView") String id, 
                        Model model) {
    //examples of addAtrribute
     = bookService.findUserById(id);
    if () {
        model.addAttribute("userFromController", user);
        return "updateUser.html";
    } else return "userNotfound.html";
}
```


3. The client renders the update-user form received from server-controller: [userForm](){.external}

```html
<form   action="#"
        th:action="@{updateBook/{userIdFromView}
        (userIdFromView=${userFromController.userId})}"
        th:object="${userFromController}"
        method="post">
    <p>
...
4. The client sends a request to update this ``user`` to ``HashMap``: [userForm](){.external} 

```html
<form   action="#"
        th:action="@{updateUser/{userIdFromView}
        (userIdFromView=${userFromController.userId})}"
        th:object="${userFromController}"
        method="post">
...
5. The sever-controller method handles the request, updates the ``user`` object in a ``HashMap`` and redirects the reponse: [UserController](){.external}

```java
@PostMapping("/updateUser/{userIdFromView}")
public String updateUser(@PathVariable("userIdFromView") String id, 
                        User user){
...
1. The client sends a ``request`` to server-controller by an ``endpoint`` to delete a user: [Users template](){.external}

2. The server-controller method handles the ``request`` and sends a response with the result. It could be success (user deleted) or fail (the deletion operation could not be done): [UserController](){.external}

3. The server-controller method after handled the request sends the response: redirects the reponse to users: [Users template](){.external}
