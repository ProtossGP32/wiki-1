...
|   | 13 | 27/09/2024 |   - Lab#Restaurant: </br> (1)  Business Logic & Controller </br> <s>(2) Entity Manager (Table, Menu)</s> </br>  (3) createOrder: test JUnit and Mockito and use-case no-table available </br> (4) Book table </br> - <s>Git</s> </br> - SCRUM: plan-execute-review-reflexionate </br></br>    | - [SCRUM: concepts](/scrum/scrum-what-concepts.qmd){.external target='_blank'}</br> - [SCRUM: guide](/scrum/scrum-what-guide.qmd){.external target='_blank'}</br> - <s>[Git](/devops/devops-vc-git.qmd){.external target='_blank'}</s> |
